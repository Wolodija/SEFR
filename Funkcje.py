...
def getWspDCT(M):
    ...
    # N = (T.H*R*T).round()+128
    return [int(round(C[0, 0]) / quant), int(round(C[0, 1]) / quant), int(round(C[1, 0]) / quant),
            int(round(C[2, 0]) / quant), int(round(C[1, 1]) / quant), int(round(C[0, 2]) / quant)]
    ...
    for i in range(R.shape[0]):
        for j in range(R.shape[1]):
            if (i * R.shape[1] + j) * 2 >= len(wiadomosc):
                break
            R[i, j] = int(
                format(R[i, j], '08b')[0:6] + wiadomosc[(i * R.shape[1] + j) * 2: (i * R.shape[1] + j) * 2 + 2], 2)
    return R

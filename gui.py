    ...
    def confDialog(self):
        ...
        try:
            ...
            for sldr in self.fraktalSldrs:
                ...
                self.fraktalSldrs[sldr].valueChanged.connect(
                    partial(self.fractalChanged, self.fraktalSldrs[sldr], sldr))
            sldr = None
            ...
            for spnb in self.fraktalSpnbs:
                ...
                self.fraktalSpnbs[spnb].valueChanged.connect(
                    partial(self.fractalChanged, self.fraktalSpnbs[spnb], spnb))
            spnb = None
...
Autorzy: Dominik Rosiek i Piotr Ścibor')
    ...
    def dodajZnakWodny(self):
        ...
        if im.size[0] % 8 or im.size[1] % 8:
            ...
            return -1
        A = usun2LSB(numpy.matrix(im.getdata()).reshape((im.size[1], im.size[0])))[
            0]  # Zamiana obrazu na macierz, zerujac 2 najmniej znaczace bity
        ...
        stat[0:int(len(R) / 2), 0:int(len(R[0]) / 2)] = Kompresuj(R[0:int(len(R) / 2), 0:int(len(R[0]) / 2)], D[3],
                                                                  delta, self)
        self.print_("Druga cwiartka")
        stat[int(len(R) / 2):, 0:int(len(R[0]) / 2)] = Kompresuj(R[int(len(R) / 2):, 0:int(len(R[0]) / 2)], D[2], delta,
                                                                 self)
        self.print_("Trzecia cwiartka")
        stat[0:int(len(R) / 2), int(len(R[0]) / 2):] = Kompresuj(R[0:int(len(R) / 2), int(len(R[0]) / 2):], D[1], delta,
                                                                 self)
        self.print_("Czwarta cwiartka")
        stat[int(len(R) / 2):, int(len(R[0]) / 2):] = Kompresuj(R[int(len(R) / 2):, int(len(R[0]) / 2):], D[0], delta,
                                                                self)
        #		print(stat)
